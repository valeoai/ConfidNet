...
class SelfConfidBCELoss(nn.modules.loss._Loss):
    ...
    def forward(self, input, target):
        ...
        return nn.BCEWithLogitsLoss(weight=weights)(confidence, (input[0].argmax(dim=1) == target).float())
...
class FocalLoss(nn.modules.loss._Loss):
    ...
    def forward(self, input, target):
        confidence = input[1].squeeze(dim=1)
        BCE_loss = F.binary_cross_entropy_with_logits(confidence, (input[0].argmax(dim=1) == target).float(),
                                                      reduction='none')
...
class StructuredMAPRankingLossFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = structured_map_ranking_loss.backward(grad_output, input, target, mask, ranking_lai)
...
class StructuredMAPRankingLoss(nn.modules.loss._Loss):
    ...
    def forward(self, input, target):
        ...
        return StructuredMAPRankingLossFunction.apply(confidence, (input[0].argmax(dim=1) == target).float()
                                                      .unsqueeze(dim=1), mask.to(dtype=torch.uint8))
...
# CUSTOM LOSSES LISTS
CUSTOM_LOSS = {'selfconfid_mse': SelfConfidMSELoss,
