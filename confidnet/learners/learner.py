...
class AbstractLeaner:
	def __init__(self, config_args, train_loader, val_loader, test_loader, start_epoch, device):
		...
		if self.task == 'segmentation':
			self.prod_train_len = self.nsamples_train * self.train_loader.dataset[0][0].shape[1] \
								  * self.train_loader.dataset[0][0].shape[2]
			self.prod_val_len = self.nsamples_val * self.val_loader.dataset[0][0].shape[1] \
								* self.val_loader.dataset[0][0].shape[2]
			self.prod_test_len = self.nsamples_test * self.test_loader.dataset[0][0].shape[1] \
	...
	def set_loss(self):
		if self.loss_args['name'] in losses.CUSTOM_LOSS:
			self.criterion = losses.CUSTOM_LOSS[self.loss_args['name']](config_args=self.config_args, device=self.device)
