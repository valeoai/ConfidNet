...
def main():
    ...
    parser.add_argument('--mode', '-m', type=str, default='normal', choices=MODE_TYPE, help='Type of confidence testing')
    parser.add_argument('--samples', '-s', type=int, default=50, help='Samples in case of MCDropout')
    ...
    if args.mode != 'trust_score':
        _, scores_test = learner.evaluate(learner.test_loader, learner.prod_test_len, split='test',
                                          mode=args.mode, samples=args.samples, verbose=True)

    # Special case TrustScore
    else:
        # For segmentation, reduce number of samples, else it is too long to compute
        if config_args['training']['task'] == 'segmentation':
            learner.prod_test_len = MAX_NUMBER_TRUSTSCORE_SEG \
                                    * np.ceil(learner.nsamples_test / config_args['training']['batch_size'])
        ...
        features_extractor.print_summary(input_size=tuple([shape_i for shape_i
                                                           in learner.train_loader.dataset[0][0].shape]))

        # Get features for KDTree
        LOGGER.info('Get features for KDTree')
        features_extractor.eval()
        metrics = Metrics(learner.metrics, learner.prod_test_len , config_args['data']['num_classes'])
        ...
        trust_model = trust_scores.TrustScore(num_workers=max(config_args['data']['num_classes'], 20))
